...
//    }
    ...
    public void swipeDown(int swipeCount) {
        try {
            ...
            int startX = dimension.width / 2;
            int endX = startX;
            int startY = (int) (dimension.height * 0.8); // Adjusted startY
            int endY = (int) (dimension.height * 0.2); // Adjusted endY

            for (int i = 0; i < swipeCount; i++) {
                TouchAction touchAction = new TouchAction<>(DriverManager.getDriver());
                touchAction.press(PointOption.point(startX, startY))
                        .waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000))) // Adjusted wait time
